...
st.sidebar.slider('Select Number of Clusters:', min_value=1, max_value=6, value=3, step=1)
...
fig_relation = px.scatter_matrix(
    df,
...
fig_elbow = px.line(x=range(1, 7), y=inertias[:6], title='Elbow Method Analysis', labels={'x': 'Number of Clusters', 'y': 'Inertia'})
...
with col1:
    ...
    fig_kmean = px.scatter(df, x='petal length (cm)', y='petal width (cm)', color='Cluster', title='KMeans Clustering Result', category_orders={'Cluster': sorted(df['Cluster'].unique())})
    st.plotly_chart(fig_kmean)


with col2:
    st.subheader('Actual Species')
    fig_species = px.scatter(df, x='petal length (cm)', y='petal width (cm)', color='Species', title='Actual Species Distribution',color_discrete_map=colors)
