...
class list
{
  ...
  public:
    ...
    void check() {
      ...
      while (n--) p = p->next;
      if (p != mHeader) {
        std::cout << mSize <<"next POINTER ERROR" << std::endl;
      }
      p = mHeader;
      n = mSize+1;
      while (n--) p = p->prev;
      if (p != mHeader) {
        std::cout << mSize << "prev POINTER ERROR" << std::endl;
      }
    }

    void reorder(int pos,std::vector<int> selected) {
      int presize = size();
      CP::list<std::string> extract;
      int currentpos=0;
      int selectpos=0;
      for(auto itr = begin();itr!=end();itr++){
        if(selectpos >= selected.size()){
          break;
        }
        if(selected[selectpos] == currentpos){
          auto thisnode = itr.ptr;
          thisnode->prev->next = thisnode->next;
          thisnode->next->prev = thisnode->prev;
          extract.push_back(thisnode->data);
          delete thisnode;
          mSize--;
          selectpos++;
        }
      ...
      int sizes = extract.size();
      if(pos==0){
        for(int i=0;i<sizes;i++){
          push_front(extract.back());
          extract.pop_back();
        }
      }else if(pos==presize){
        for(int i=0;i<sizes;i++){
          push_back(extract.front());
          extract.pop_front();
        }
      }else{
        auto itr = begin();
